...
data_cseal = data[data.label == 'Cracked_Seal']
data_norm = data[data.label == 'Normal']

data = pd.concat([data_cseal, data_norm], ignore_index=True)
...
param_grid = {'svc__kernel': ['rbf'],
             'svc__class_weight': [{'Normal':1, 'Cracked_Seal':25}],
              'svc__C': [1]
             }
...
for i in range(len(dec_func)):
    if dec_func[i] < 0 and dec_func[i] > -1:
        count += 1
        indexes.append(i)
...
cracked_seal_data = pd.concat([data_X,data_Y], axis=1)

# This code segment converts the datetime column to a datetime object that can be used to sort chronologically

cracked_seal_data.datetime = pd.to_datetime(test_data.datetime, yearfirst=True)
cracked_seal_data.sort_values(by='datetime')
...
scaler = StandardScaler()
clf = SVC(C=1, class_weight={'Normal':1,'Cracked_Seal':25, 'Yellow':25},kernel="rbf")
pipe = Pipeline(steps=[('scaler', scaler), ('svc', clf)])
