...
data = pd.read_csv("labeled_data_l_valve.csv", names = col_names)
data = data[data.motor_stat != 'Off']
data_lval = data[data.label == 'Leaking_Valve']
data_norm = data[data.label == 'Normal']

data = pd.concat([data_lval, data_norm], ignore_index=True)
...
param_grid = {'svc__kernel': ['rbf'],
             'svc__class_weight': [{'Normal':1, 'Leaking_Valve':25}],
              'svc__C': [1]
             }
...
for i in range(len(data_Y)):
    if i in indexes:
        data_Y[i] = "Leaking_Valve_Warning"
...
scaler = StandardScaler()
clf = SVC(C=1, class_weight={'Normal':1,'Leaking_Valve':25, 'Yellow':25},kernel="rbf")
pipe = Pipeline(steps=[('scaler', scaler), ('svc', clf)])
