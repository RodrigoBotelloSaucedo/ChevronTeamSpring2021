...
data_bval = data[data.label == 'Broken_Valve']
data_norm = data[data.label == 'Normal']

data = pd.concat([data_bval, data_norm], ignore_index=True)
...
param_grid = {'svc__kernel': ['rbf'],
             'svc__class_weight': [{'Normal':1, 'Broken_Valve':10 },
                                   {'Normal':1, 'Broken_Valve':20 },
                                   'balanced'],
              'svc__C': [1,5,10,15]
             }
...
param_grid = {'svc__kernel': ['rbf'],
             'svc__class_weight': [{'Normal':1, 'Broken_Valve':25}],
              'svc__C': [1]
             }
...
scaler = StandardScaler()
clf = SVC(C=1, class_weight={'Normal':1,'Broken_Valve':25, 'Yellow':25},kernel="rbf")
pipe = Pipeline(steps=[('scaler', scaler), ('svc', clf)])
