...
data_bimp = data[data.label == 'Broken_Impeller']
data_norm = data[data.label == 'Normal']

data = pd.concat([data_bimp, data_norm], ignore_index=True)
...
param_grid = {'svc__kernel': ['rbf'],
             'svc__class_weight': [{'Normal':1, 'Broken_Impeller':20},
                                   {'Normal':1, 'Broken_Impeller':25},
                                   {'Normal':1, 'Broken_Impeller':30},
                                   'balanced'],
              'svc__C': [5,10,15]
             }
...
param_grid = {'svc__kernel': ['rbf'],
             'svc__class_weight': [{'Normal':1, 'Broken_Impeller':25}],
              'svc__C': [5]
             }
...
# This code relabels the data point within the uncertainty margin as "Broken_Impeller_Warning"

for i in range(len(data_Y)):
    if i in indexes:
        data_Y[i] = "Broken_Impeller_Warning"
...
scaler = StandardScaler()
clf = SVC(C=0.1, class_weight={'Normal':1,'Broken_Impeller':25, 'Yellow':25},kernel="rbf")
pipe = Pipeline(steps=[('scaler', scaler), ('svc', clf)])
